...
- **Web substitution:** SF Symbols are proprietary and can't ship, so this
  ...
  `@phosphor-icons/web` jsDelivr CSS.
- **Use the `Icon` component.** Don't hand-write `<i class="ph-…">` — the
  `Icon` component (`components/core/Icon.jsx`) wraps Phosphor with the brand
  defaults (fill weight, `currentColor`, decorative-by-default a11y):
  `<Icon name="car" />`, `<Icon name="caret-right" weight="bold" />`,
  `<Icon name="map-pin" label="Location" />`. Host pages load the `fill` and
  `bold` Phosphor weight stylesheets.
- **Browse the full set:** the **Icon library** card
  (`components/core/card-icons.html`, on the Design System tab) shows every
  glyph the app uses — categories, tabs/navigation, actions, travel/status/
  attachments — each labeled with its Phosphor name.
...
- `Icon` — brand-defaulted glyph over Phosphor (the SF Symbols stand-in).
